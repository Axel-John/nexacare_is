...
def dashboard_ui(page: ft.Page, user: dict):
    ...
    header = ft.Container(
        content=ft.Row(
            [
                ft.Text(f"Welcome, Dr. {user.get('name', 'User')}", size=24, weight="bold"),
    ...
    content = ft.Container(
        content=ft.Column(
            [
                ft.Text("Doctor Dashboard Content", size=20),
                ft.Text("Your appointments and patient records will appear here.", size=16)
            ],
            spacing=20
        ),
        padding=20,
